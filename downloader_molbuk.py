...
class Downloader(object):
    def __init__(self, rootPath):
        ...
        self.getLinksCmd = (
            downloader_common.XIDEL_CMD +
            ' --xpath \'//table[@class="main-table"]//td[@class="td-for-content"]//div[@class="short-1"]//div[@class="short-1-title"]//@href\' '  # href
            ' --xpath \'//table[@class="main-table"]//td[@class="td-for-content"]//div[@class="short-1"]//div[@class="short-1-more"]//span\' '  # time
            ' --output-format=json-wrapped --output-encoding=windows-1251'
        )  # output as json
        ...
        try:
            if len(jsonArt) > 0:
                ...
                cmdContent = (
                    downloader_common.XIDEL_CMD.format(url) +
                    ' --xpath \'//table[@class="main-table"]//td[@class="td-for-content"]//div[@class="short-block"]//div[@class="shot-text"]\''  #article text
                    ' --output-format=html --output-encoding=windows-1251'
                )  # output as html
    ...
    def loadArticleTextFromHtml(self, xidelCmd):
        ...
        # print(result)
        txt = result.replace('<br>', '[br]').replace('</br>', '').replace(
            '<p>', '[br]').replace('</p>', '').replace('<br />', '[br]')
        txt = txt.replace('<BR>', '[br]').replace('</BR>', '').replace(
            '<P>', '[br]').replace('</P>', '').replace('<BR />', '[br]')
        txt = txt.replace('&amp;#', '&#')
    ...
    def fb2(self, date, articleList):
        ...
        ret += '\n  <book-title>Молодий буковинець. Новини. ' + date.strftime(
            '%d.%m.%Y') + '</book-title>'
        ret += '\n  <date>' + str(date) + '</date>'
        ret += '\n  <lang>uk</lang>'
        ret += '\n </title-info>'
    ...
    def getLastDownloadedIssueDate(self):
        now = datetime.datetime.now()
        curYearFolder = downloader_common.rootPath + '/molbuk/' + str(now.year)
        prevYearFolder = downloader_common.rootPath + '/molbuk/' + str(
            now.year - 1)
        lastIssueFolder = downloader_common.rootPath + '/molbuk'
        ...
        for issueFile in os.listdir(lastIssueFolder):
            if issueFile.endswith(".fb2"):
                curIssueDate = datetime.datetime.strptime(
                    issueFile[len('molbuk'):-4], '%Y-%m-%d').date()
                if curIssueDate > lastIssueDate:
                    lastIssueDate = curIssueDate


downloader = Downloader(downloader_common.rootPath)
logging.basicConfig(
    filename='downloader_molbuk.log',
    level=logging.INFO,
