...
def run():
    ...
    while (date < dateTo):
      ...
      if len(content) > 0:
        with open(rootPath+'/unian/'+str(date.year)+'/unian_'+str(date)+'.fb2', "w") as fb2_file:
...
class Downloader(object):
  ...
  def __init__(self, rootPath):
    ...
    self.getLinksCmd = downloader_common.XIDEL_CMD + ' --xpath \'//div[@class="gallery-container masonry"]//a[@class="publication-title"]/@href\''
  ...
  def getNewsForDate(self, date):
    ...
    for ln in p.stdout:
      ...
      line = line.replace("http://culture.unian.net/", "http://culture.unian.ua/")
      line = line.replace("http://crimea.unian.net/", "http://crimea.unian.ua/")
      if line.startswith("//"):
          line = 'https:'+line
      ...
      if len(line) > 0 and (line.startswith("http://") or line.startswith("https://")) and (".unian.ua/" in line or "unian.net/ukr" in line):
  ...
  def loadArticle(self, url):
    cmd = (downloader_common.XIDEL_CMD.format(url) +
           ' --xpath \'//section[@class="article-column"]//div[@class="meta"]//time[@itemprop="datePublished"]/@content\'' #datetime in format yyyy-mm-dd hh:mi:ss
           ' --xpath \'//div[@class="article-text"]//h1[@itemprop="headline"]\'' #title
      ...
      if len(article.body) <= 1 : #article has only one row, download as html
          ...
          if not bodyRead:
              if len(text) > 0: #article is not empty
                ...
                cmd = (downloader_common.XIDEL_CMD.format(url) +
                    ' --xpath \'//div[@class="article-text"]//span[@itemprop="articleBody"]//div\'') #article text
                p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
                result = p.communicate()[0].decode('utf-8')
                if len(str(result).strip()) == 0 :
                    cmd = (downloader_common.XIDEL_CMD.format(url) +
                       ' --xpath \'//div[@class="article-text"]//p\'') #article text
                    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
                    result = p.communicate()[0].decode('utf-8')
                jsonArt[3] = result
  ...
  def fb2(self, date):
    ...
    ret += '\n  <book-title>УНІАН. Новини ' + date.strftime('%d.%m.%Y') + '</book-title>'
    ret += '\n  <date>' + str(date) + '</date>'
    ret += '\n  <lang>uk</lang>'
    ret += '\n </title-info>'
    ret += '\n <document-info>'
  ...
  def load(self, sDateFrom, sDateTo):
    ...
    while (date < dateTo):
      ...
      if len(content) > 0:
        with open(self.rootPath+'/unian/'+str(date.year)+'/unian_'+str(date)+'.fb2', "w") as fb2_file:
          fb2_file.write(content)
      date += datetime.timedelta(days=1)
