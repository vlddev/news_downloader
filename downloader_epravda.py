...
class Downloader(object):
  ...
  def __init__(self, rootPath):
    self.baseUrl = 'http://www.epravda.com.ua'
    self.getLinksCmd = downloader_common.XIDEL_CMD + ' --xpath \'//div[@class="block_archive_list"]//a/@href\''
  ...
  def loadArticle(self, url):
    ...
    cmd = ('xidel '+url+' -q '
           ' --xpath \'//div[@class="rpad"]//span[@class="dt2"]\'' #date
           ' --xpath \'//div[@class="rpad"]//h1[@class="title"]\'' #title
           ' --xpath \'//div[@class="rpad"]//div[@class="dummy"]\'' #article text
    ...
    cmd = (downloader_common.XIDEL_CMD.format(url) +
           ' --xpath \'//header[@class="post__header"]//div[@class="post__time"]\'' #date
           ' --xpath \'//header[@class="post__header"]//h1[@class="post__title"]\'' #title
           ' --xpath \'//div[@class="block_post"]//div[@class="dummy"]\'' #article dummy text
           ' --output-format=json-wrapped' #output as json
           ' --output-encoding=windows-1251')  #pravda.com.ua uses encoding=windows-1251
    #print('cmd: '+cmd)
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
  ...
  def fb2(self, date):
    ...
    ret += '\n  <book-title>Економічна правда. Новини ' + date.strftime('%d.%m.%Y') + '</book-title>'
    ret += '\n  <date>' + str(date) + '</date>'
    ret += '\n  <lang>uk</lang>'
    ret += '\n </title-info>'
    ret += '\n <document-info>'
  ...
  def load(self, sDateFrom, sDateTo):
    ...
    while (date < dateTo):
      ...
      if len(content) > 0:
        with open(self.rootPath+'/epravda/'+str(date.year)+'/epravda_'+str(date)+'.fb2', "w") as fb2_file:
          fb2_file.write(content)
      date += datetime.timedelta(days=1)
